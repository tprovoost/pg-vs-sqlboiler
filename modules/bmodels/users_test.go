...
func testUserToManyPurchases(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, purchaseDBTypes, false, purchaseColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, purchaseDBTypes, false, purchaseColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Purchases().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPurchases(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Purchases); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Purchases = nil
	if err = a.L.LoadPurchases(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Purchases); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpPurchases(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e Purchase

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Purchase{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, purchaseDBTypes, false, strmangle.SetComplement(purchasePrimaryKeyColumns, purchaseColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Purchase{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddPurchases(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Purchases[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Purchases[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Purchases().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpPurchases(t *testing.T) {
	var err error
	...
	err = a.SetPurchases(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetPurchases(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Purchases[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Purchases[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpPurchases(t *testing.T) {
	var err error
	...
	err = a.AddPurchases(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePurchases(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Purchases) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Purchases[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Purchases[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Email`: `character varying`, `Password`: `character varying`, `Details`: `hstore`, `CreatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
