...
type User struct {
	...
	Details   types.HStore `boil:"details" json:"details,omitempty" toml:"details" yaml:"details,omitempty"`
...
type whereHelpertypes_HStore struct{ field string }

func (w whereHelpertypes_HStore) EQ(x types.HStore) qm.QueryMod {
	return qmhelper.WhereNullEQ(w.field, false, x)
}
func (w whereHelpertypes_HStore) NEQ(x types.HStore) qm.QueryMod {
	return qmhelper.WhereNullEQ(w.field, true, x)
}
func (w whereHelpertypes_HStore) IsNull() qm.QueryMod    { return qmhelper.WhereIsNull(w.field) }
func (w whereHelpertypes_HStore) IsNotNull() qm.QueryMod { return qmhelper.WhereIsNotNull(w.field) }
func (w whereHelpertypes_HStore) LT(x types.HStore) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.LT, x)
}
func (w whereHelpertypes_HStore) LTE(x types.HStore) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.LTE, x)
}
func (w whereHelpertypes_HStore) GT(x types.HStore) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.GT, x)
}
func (w whereHelpertypes_HStore) GTE(x types.HStore) qm.QueryMod {
...
var UserWhere = struct {
	...
	Details   whereHelpertypes_HStore
	...
	Details:   whereHelpertypes_HStore{field: "\"users\".\"details\""},
...
var (
	userAllColumns            = []string{"id", "email", "password", "details", "created_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"email", "password", "details", "created_at", "deleted_at"}
...
func (o *User) Purchases(mods ...qm.QueryMod) purchaseQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"purchases\".\"user_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"purchases\".*"})
	}
...
func (userL) LoadPurchases(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`purchases`),
		qm.WhereIn(`purchases.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load purchases")
	}

	var resultSlice []*Purchase
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice purchases")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on purchases")
	}
...
func (o *User) AddPurchases(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Purchase) error {
	var err error
...
// SetPurchases removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Purchases accordingly.
// Replaces o.R.Purchases with related.
// Sets related.R.User's Purchases accordingly.
func (o *User) SetPurchases(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Purchase) error {
	query := "update \"purchases\" set \"user_id\" = null where \"user_id\" = $1"
	...
	return o.AddPurchases(ctx, exec, insert, related...)
}

// RemovePurchases relationships from objects passed in.
// Removes related items from R.Purchases (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemovePurchases(ctx context.Context, exec boil.ContextExecutor, related ...*Purchase) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.UserID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Purchases {
			if rel != ri {
				continue
			}

			ln := len(o.R.Purchases)
			if ln > 1 && i < ln-1 {
				o.R.Purchases[i] = o.R.Purchases[ln-1]
			}
			o.R.Purchases = o.R.Purchases[:ln-1]
			break
		}
	}
